...
const wrap = (inst) => {
  var obj = {
    ...
    validateOrderAuthorization: (hash, maker, sig, misc) => {
      console.log( web3.eth.abi.encodeParameters(['uint8', 'bytes32', 'bytes32'], [sig.v, sig.r, sig.s]) + (sig.suffix || ''));
      return inst.validateOrderAuthorization_.call(hash, maker, web3.eth.abi.encodeParameters(['uint8', 'bytes32', 'bytes32'], [sig.v, sig.r, sig.s]) + (sig.suffix || ''), misc);
    },
